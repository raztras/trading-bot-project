...
from config import SYMBOL, FILES_DIR, MASTER, COIN

load_dotenv()

def fetch_historical_prices(days, resample_minutes=1, exchange_id="binance"):
    """
    Fetch OHLCV from an exchange via ccxt. Defaults to Binance BTC/USDT.
    Returns a DataFrame indexed by naive Europe/London timestamps with columns open, high, low, close.
...
def save_to_files(df, coin, currency, days):
    """
    Orchestrates resampling and metric calculation, then writes CSV for the requested granularity.
...
def save_to_master(coin=COIN, currency="usd", days=10):
    """
    ...
    last_df.to_csv(MASTER, mode="a", index=False, header=write_header)
    return logger.info(f"Appended latest row to master file: {MASTER}")
