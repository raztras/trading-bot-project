...
def fetch_historical_prices(days, resample_minutes=5, exchange_id="binance", max_retries=3):
    """
    Fetch OHLCV from an exchange via ccxt. Defaults to Binance BTC/USDT.

    Args:
        days: Number of days of historical data to fetch
        resample_minutes: Candle interval in minutes (default: 5)
        exchange_id: Exchange to fetch from (default: "binance")
        max_retries: Maximum number of retry attempts on failure (default: 3)

    Returns:
        DataFrame indexed by naive Europe/London timestamps with columns
        open, high, low, close

    Raises:
        ccxt.NetworkError: If network connection fails after retries
        ccxt.ExchangeError: If exchange returns an error
        ValueError: If invalid exchange_id or symbol provided
    """
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ccxt.exchanges for available options."
        )
    ...
    while True:
        try:
            ...
            if retries >= max_retries:
                ...
                raise
            logger.warning(f"Network error, retry {retries}/{max_retries}: {e}")
    ...
    try:
        ...
        raise ValueError(f"Failed to process data from {exchange_id}: {e}")
